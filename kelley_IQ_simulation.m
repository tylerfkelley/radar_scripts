...
window_hann = blackmanharris(length(real(beat_signal)));
window_hann = 1;
real_beat_signal = real(beat_signal) .* window_hann';

S_beat_FFT = abs(fft(real_beat_signal, N_fft)); % Compute FFT magnitude
...
subplot(2,1,2)
plot(t.*1e6, atan(real(beat_signal) ./ imag(beat_signal)), 'b', 'LineWidth', 2);
xlabel("$$\bf{Time\: (us)}$$", 'FontWeight', 'bold', 'FontSize', FONTSIZE, 'Interpreter', 'latex');
ylabel("$$\bf{Phase}$$", 'FontWeight', 'bold', 'FontSize', FONTSIZE, 'Interpreter', 'latex');
title("$$\bf{Range\: Spectrum}$$", 'FontWeight', 'bold', 'FontSize', FONTSIZE, 'Interpreter', 'latex');
grid on;
